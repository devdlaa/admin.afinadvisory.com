...
const ReferredByTypeEnum = z.enum(["entity", "influencer", "external"]);
...
export const listTasksSchema = z
  .object({
    ...
    referred_by_type: ReferredByTypeEnum.optional(),

    referred_by_search_field: z.string().trim().min(1).optional(),
  ...
  .superRefine((data, ctx) => {
    if (data.referred_by_type && !data.referred_by_search_field) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["referred_by_search_field"],
        message:
          "referred_by_search_field is required when referred_by_type is provided",
      });
    }
  })
  .transform((data) => {
    if (
      data.referred_by_type !== "external" ||
      !data.referred_by_search_field
    ) {
      return data;
    }

    const value = data.referred_by_search_field.trim();

    let referred_by_search_kind = "NAME";

    if (z.string().email().safeParse(value).success) {
      referred_by_search_kind = "EMAIL";
    } else if (/^\+?[0-9]{7,15}$/.test(value)) {
      referred_by_search_kind = "PHONE";
    }

    return {
      ...data,
      referred_by_search_kind,
    };
  });

const TaskReferenceUpdateSchema = z
  .object({
    type: z.enum(["entity", "influencer", "external", "lead_contact"]),
...
export const TaskUpdateSchema = z
  .object({
    ...
    reference: TaskReferenceUpdateSchema,
    due_date: z
...
export const toggleBoardPin = z.object({
  category_id: z.string().uuid(),
    new_position: z.number().int().positive().optional(),
});