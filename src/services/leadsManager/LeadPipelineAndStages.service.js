...
export async function deleteLeadPipeline(id, admin_user_id) {
  ...
  if (pipeline.is_default) {
    throw new ValidationError("Default pipeline cannot be deleted");
  }
...
export async function deleteLeadPipelineStage(
  ...
  return prisma.$transaction(async (tx) => {
    ...
    /* ─── 5. Fetch affected leads ─────────────────── */
    const affectedLeads = await tx.$queryRaw`
      SELECT id
      FROM "Lead"
    ...
    /* ─── 7. Update Lead Stage Hisotry ───────────────────────────── */
    if (affectedLeads.length > 0) {
      await tx.$executeRaw`
        INSERT INTO "LeadStageHistory" (
          id,
          lead_id,
          from_stage_id,
          to_stage_id,
        ...
        SELECT
          ...
          ${admin_user_id}::uuid,
          NOW(),
          'Bulk migration due to stage deletion'
        FROM jsonb_to_recordset(${JSON.stringify(affectedLeads)}::jsonb)
        AS x(id uuid)
      `;
    }

    /* ─── 8. Insert bulk operation log ───────────── */
    if (count > 0) {
      await tx.$queryRaw`
