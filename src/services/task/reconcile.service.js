...
export async function getUnreconciledTasks(filters, options = {}) {
  ...
  const query = {
    ...
    include: taskInclude,
    orderBy: {
      created_at: filters.order === "asc" ? "asc" : "desc",
    },
  };
  ...
  const items = tasks.map((task) => ({
    ...
    task: mapTask(task),
    reference:
      options.exportMode && task.reference
        ? {
            type: task?.reference?.type || null,
            id:
              task.reference.entity_id || task.reference.influencer_id || null,
            name: task.reference.entity_id
              ? task.reference.entity.name
              : task.reference.influencer
                ? task.reference.influencer.name
                : task.reference.name,
            email: task.reference.entity_id
              ? task.reference.entity.email
              : task.reference.influencer
                ? task.reference.influencer.email
                : task.reference.email,
            phone: task.reference.entity_id
              ? task.reference.entity.primary_phone
              : task.reference.influencer
                ? task.reference.influencer.phone
                : task.reference.phone,
          }
        : null,
