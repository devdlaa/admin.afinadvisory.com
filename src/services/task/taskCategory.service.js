...
export const deleteTaskCategory = async (category_id) => {
  return prisma.$transaction(async (tx) => {
    ...
    // (cascade will delete the pin rows, so we reorder before deletion)
    for (const pin of category.pinned_by_users) {
      await tx.adminUserPinnedBoard.updateMany({
        where: {
          admin_user_id: pin.admin_user_id,
          position: { gt: pin.position },
        },
        data: {
...
export const listTaskCategories = async (filters = {}, adminUserId) => {
  ...
  return {
    data: items.map(({ pinned_by_users, ...item }) => ({
      ...item,
      is_pinned: pinned_by_users ? pinned_by_users.length > 0 : false,
      pin_position: pinned_by_users?.[0]?.position ?? null,
    })),
    pagination: {
      page,
      page_size: pageSize,
