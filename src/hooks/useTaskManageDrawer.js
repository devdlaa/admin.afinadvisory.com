...
import {
  ...
  selectQuickSearchResults as selectQuickSearchResultsLeadContact,
  selectQuickSearchLoading as selectQuickSearchLoadingLeadContact,
...
export function useTaskManageDrawer() {
  ...
  const contactResults = useSelector(selectQuickSearchResultsLeadContact);
  const contactSearching = useSelector(selectQuickSearchLoadingLeadContact);
  ...
  const entityResults = useSelector(selectQuickSearchResults);
  const entityLoadingStates = useSelector(selectEntityLoadingStates);
  const entitySearching = entityLoadingStates?.quickSearch ?? false;

  // ── Assignment dialog state ────────────────
  const [showAssignmentDialog, setShowAssignmentDialog] = useState(false);
  const normalizeReferenceForDisplay = (ref) => {
    if (!ref) return null;

    const TYPE_LABELS = {
      ENTITY: "ENTITY",
      INFLUENCER: "INFLUENCER",
      EXTERNAL_PERSON: "EXTERNAL CONTACT",
    };
    ...
    return {
      __type: TYPE_MAP[ref.type],
      id: ref.id,
      name: ref.name,
      email: ref.email,
      phone: ref.phone,
      displayName: ref.name,
      displayMeta: [ref.email, ref.phone].filter(Boolean).join(" • "),
      typeLabel: TYPE_LABELS[ref.type],
    };
  };
  ...
  useEffect(() => {
    ...
    setSelectedReferenceData(normalizeReferenceForDisplay(task.reference));
  }, [task]);
  ...
  // taks reference handlers
  const handleReferenceSearch = (query, tabId) => {
    if (!query.trim()) return;

    if (tabId === "entity") {
      dispatch(quickSearchEntities({ search: query, limit: 20 }));
    }
  ...
  const handleReferenceConfirm = async (payload) => {
    try {
      ...
      if (payload.type === "__cleared") {
        ...
        reference = { type: "entity", id: payload.id };
      } else if (payload.type === "influencer") {
        reference = { type: "influencer", id: payload.id };
      ...
      const result = await dispatch(
        updateTask({ taskId: task.id, data: { reference } }),
      ).unwrap();

      setSelectedReferenceData(
        normalizeReferenceForDisplay(result?.task?.reference),
      );
