...
const toastMiddleware = () => (next) => (action) => {
  ...
  if (action.type.endsWith("/fulfilled")) {
    switch (action.type) {
      ...
      case "activities/updateActivityLifecycle/fulfilled":
        showSuccess("Activity status updated successfully");
        break;

      case "activities/updateActivityEmail/fulfilled":
        showSuccess("Email updated successfully");
        break;

      // Silent reads
      case "activities/fetch/fulfilled":
      case "activities/fetchActivityEmail/fulfilled":
        break;

      case "reconcile/exportTasks/fulfilled":
        showSuccess("Export downloaded successfully");
        break;

      // ===== LEAD DETAILS SLICE FULFILLED =====

      case "leadDetails/updateLead/fulfilled":
        showSuccess("Lead updated successfully");
        break;

      case "leadDetails/updateLeadStage/fulfilled": {
        const stageName = action.payload?.stage?.name;
        showSuccess(
          stageName
      ...
      case "leadDetails/updateLeadAssignments/fulfilled": {
        const assignCount = action.payload?.assignments?.length ?? 0;
        showSuccess(
          `${assignCount} user${assignCount !== 1 ? "s" : ""} assigned successfully`,
        );
        break;
      }

      case "leadDetails/createLeadActivity/fulfilled":
        showSuccess("Activity created successfully");
        break;

      case "leadDetails/updateLeadActivity/fulfilled":
        showSuccess("Activity updated successfully");
        break;

      case "leadDetails/updateActivityLifecycle/fulfilled":
        showSuccess("Activity status updated successfully");
        break;

      case "leadDetails/deleteActivity/fulfilled":
        showSuccess("Activity deleted successfully");
        break;

      case "leadDetails/updateActivityEmail/fulfilled":
        showSuccess("Email updated successfully");
        break;

      // Silent reads
      case "leadDetails/fetchLeadDetails/fulfilled":
      case "leadDetails/fetchAiSummary/fulfilled":
      case "leadDetails/fetchTeamEffort/fulfilled":
      case "leadDetails/fetchLeadActivities/fulfilled":
      case "leadDetails/fetchActivityEmail/fulfilled":
        break;

      // ===== LEAD CONTACTS SLICE FULFILLED =====

      case "leadContact/create/fulfilled": {
        const contactName =
          action.payload?.contact_person ||
          action.payload?.company_name ||
          "Contact";
        showSuccess(`${contactName} added successfully`);
        break;
      }

      case "leadContact/update/fulfilled": {
        const updatedName =
          action.payload?.contact_person ||
          action.payload?.company_name ||
          "Contact";
        showSuccess(`${updatedName} updated successfully`);
        break;
      }

      case "leadContact/delete/fulfilled":
        showSuccess("Contact deleted successfully");
        break;

      // Silent reads
      case "leadContact/fetchList/fulfilled":
      case "leadContact/quickSearch/fulfilled":
      case "leadContact/fetchById/fulfilled":
        break;

      // ===== LEAD PIPELINES SLICE FULFILLED =====

      case "leadPipelines/create/fulfilled": {
        const pipeline = action.payload?.pipeline ?? action.payload;
        const pipelineName = pipeline?.name || "Pipeline";
        showSuccess(`"${pipelineName}" created successfully`);
        break;
      }

      case "leadPipelines/update/fulfilled": {
        const updatedPipelineName = action.payload?.name || "Pipeline";
        showSuccess(`"${updatedPipelineName}" updated successfully`);
        break;
      }

      case "leadPipelines/delete/fulfilled":
        showSuccess("Pipeline deleted successfully");
        break;

      case "leadPipelines/deleteStage/fulfilled":
        showSuccess("Stage deleted successfully");
        break;

      // Silent reads
      case "leadPipelines/fetchList/fulfilled":
      case "leadPipelines/fetchById/fulfilled":
      case "leadPipelines/fetchAndSetActive/fulfilled":
      case "leadPipelines/searchLeads/fulfilled":
        break;

      // ===== LEADS META (TAGS) SLICE FULFILLED =====

      case "leadsMeta/createTag/fulfilled": {
        const tagName = action.payload?.tag?.name || "Tag";
        showSuccess(`"${tagName}" created successfully`);
        break;
      }

      case "leadsMeta/updateTag/fulfilled": {
        const updatedTagName = action.payload?.tag?.name || "Tag";
        showSuccess(`"${updatedTagName}" updated successfully`);
        break;
      }

      case "leadsMeta/deleteTag/fulfilled":
        showSuccess("Tag deleted successfully");
        break;

      // Silent reads
      case "leadsMeta/fetchTags/fulfilled":
        break;

      // ===== LEADS (KANBAN) SLICE FULFILLED =====

      case "leads/createLead/fulfilled": {
        const leadTitle = action.payload?.title || "Lead";
        showSuccess(`"${leadTitle}" created successfully`);
        break;
      }

      // Silent (optimistic UI handles feedback for drag-and-drop)
      case "leads/fetchPipelineLeads/fulfilled":
      case "leads/updateLeadStage/fulfilled":
        break;

      // ===== LEAD TIMELINE SLICE FULFILLED =====

      case "leadTimeline/createComment/fulfilled":
        showSuccess("Comment added successfully");
        break;

      case "leadTimeline/updateComment/fulfilled":
        showSuccess("Comment updated successfully");
        break;

      case "leadTimeline/deleteComment/fulfilled":
        showSuccess("Comment deleted successfully");
        break;

      // Silent reads
      case "leadTimeline/fetchTimeline/fulfilled":
      case "leadTimeline/loadMoreTimeline/fulfilled":
      case "leadTimeline/fetchStageHistory/fulfilled":
        break;

      // ===== LEADS INFLUENCER SLICE (leads-manager) FULFILLED =====

      case "influencers/create/fulfilled": {
        const newInfluencerName =
          action.payload?.name || action.payload?.username || "Influencer";
        showSuccess(`${newInfluencerName} added successfully`);
        break;
      }

      case "influencers/update/fulfilled": {
      ...
      case "influencers/delete/fulfilled":
        showSuccess("Influencer deleted successfully");
        break;

      // Silent reads
      case "influencers/fetchList/fulfilled":
      case "influencers/search/fulfilled":
      case "influencers/fetchById/fulfilled":
        break;
      ...
      case "taskCategory/toggleCategoryPin/fulfilled":
        const pinResult = action.payload;
        showSuccess(
          pinResult.is_pinned
      ...
      case "taskCategory/fetchCategoryBoards/fulfilled":
        // Silent
        break;
  ...
  if (action.type.endsWith("/rejected")) {
    switch (action.type) {
      // ===== ACTIVITIES SLICE (leads-manager global list) REJECTED =====

      case "activities/fetch/rejected":
        showError("Failed to load activities. Please refresh and try again.");
        break;
      ...
      case "activities/updateActivityLifecycle/rejected": {
        const err = getErrorMessage(action);
        showError(err || "Failed to update activity status");
        break;
      }

      case "activities/fetchActivityEmail/rejected":
        showError("Failed to load email details. Please try again.");
        break;

      case "activities/updateActivityEmail/rejected": {
        const err = getErrorMessage(action);
        showError(err || "Failed to update email");
        break;
      }

      // ===== LEAD DETAILS SLICE REJECTED =====

      case "leadDetails/fetchLeadDetails/rejected": {
        const err = getErrorMessage(action);
        if (safeIncludes(err, "not found")) {
          showError("Lead not found. It may have been deleted.");
        } else {
          showError("Failed to load lead details. Please try again.");
        }
        break;
      }

      case "leadDetails/updateLead/rejected": {
        const err = getErrorMessage(action);
        if (safeIncludes(err, "not found")) {
          showError("Lead not found. Please refresh the page.");
        } else if (safeIncludes(err, "validation")) {
      ...
      case "leadDetails/updateLeadStage/rejected": {
        const err = getErrorMessage(action);
        showError(err || "Failed to update lead stage");
        break;
      }

      case "leadDetails/updateLeadAssignments/rejected": {
        const err = getErrorMessage(action);
        showError(err || "Failed to update assignments");
        break;
      }

      case "leadDetails/fetchAiSummary/rejected":
        showError("Failed to generate AI summary. Please try again.");
        break;

      case "leadDetails/fetchTeamEffort/rejected":
        showError("Failed to load team effort data.");
        break;

      case "leadDetails/fetchLeadActivities/rejected":
        showError("Failed to load activities. Please try again.");
        break;

      case "leadDetails/createLeadActivity/rejected": {
        const err = getErrorMessage(action);
        if (safeIncludes(err, "validation")) {
          showError("Invalid activity data. Please check your input.");
        } else {
          showError(err || "Failed to create activity");
        }
        break;
      }

      case "leadDetails/updateLeadActivity/rejected": {
        const err = getErrorMessage(action);
        showError(err || "Failed to update activity");
        break;
      }

      case "leadDetails/updateActivityLifecycle/rejected": {
        const err = getErrorMessage(action);
        showError(err || "Failed to update activity status");
        break;
      }

      case "leadDetails/deleteActivity/rejected": {
        const err = getErrorMessage(action);
        showError(err || "Failed to delete activity");
        break;
      }

      case "leadDetails/fetchActivityEmail/rejected":
        showError("Failed to load email details. Please try again.");
        break;

      case "leadDetails/updateActivityEmail/rejected": {
        const err = getErrorMessage(action);
        showError(err || "Failed to update email");
        break;
      }

      // ===== LEAD CONTACTS SLICE REJECTED =====

      case "leadContact/fetchList/rejected":
        showError("Failed to load contacts. Please refresh and try again.");
        break;

      case "leadContact/quickSearch/rejected":
        showError("Contact search failed. Please try again.");
        break;

      case "leadContact/fetchById/rejected": {
        const err = getErrorMessage(action);
        if (safeIncludes(err, "not found")) {
          showError("Contact not found. Please refresh the page.");
        } else {
      ...
      case "leadContact/create/rejected": {
        const err = getErrorMessage(action);
        if (
          safeIncludes(err, "duplicate") ||
          safeIncludes(err, "already exists")
        ) {
          showError("A contact with this email or phone already exists.");
        } else if (safeIncludes(err, "validation")) {
          showError("Invalid contact data. Please check your input.");
        } else {
          showError(err || "Failed to add contact");
        }
        break;
      }

      case "leadContact/update/rejected": {
        const err = getErrorMessage(action);
        if (safeIncludes(err, "not found")) {
          showError("Contact not found. Please refresh the page.");
        } else if (
          ...
          showError("Update failed: email or phone already in use.");
        } else {
      ...
      case "leadContact/delete/rejected": {
        const err = getErrorMessage(action);
        showError(err || "Failed to delete contact");
        break;
      }

      // ===== LEAD PIPELINES SLICE REJECTED =====

      case "leadPipelines/fetchList/rejected":
        showError("Failed to load pipelines. Please refresh and try again.");
        break;

      case "leadPipelines/fetchById/rejected":
      case "leadPipelines/fetchAndSetActive/rejected": {
        const err = getErrorMessage(action);
        if (safeIncludes(err, "not found")) {
          showError("Pipeline not found. Please refresh the page.");
        } else {
          showError("Failed to load pipeline. Please try again.");
        }
        break;
      }

      case "leadPipelines/create/rejected": {
        const err = getErrorMessage(action);
        if (
          safeIncludes(err, "duplicate") ||
          safeIncludes(err, "already exists")
        ) {
          showError("A pipeline with this name already exists.");
        } else {
          showError(err || "Failed to create pipeline");
        }
        break;
      }

      case "leadPipelines/update/rejected": {
        const err = getErrorMessage(action);
        if (safeIncludes(err, "not found")) {
          showError("Pipeline not found. Please refresh the page.");
        } else {
          showError(err || "Failed to update pipeline");
        }
        break;
      }

      case "leadPipelines/delete/rejected": {
        const err = getErrorMessage(action);
        if (safeIncludes(err, "associated") || safeIncludes(err, "has leads")) {
          showError("Cannot delete pipeline: it still has active leads.");
        } else {
          showError(err || "Failed to delete pipeline");
        }
        break;
      }

      case "leadPipelines/deleteStage/rejected": {
        const err = getErrorMessage(action);
        if (safeIncludes(err, "has leads") || safeIncludes(err, "associated")) {
          showError("Cannot delete stage: migrate its leads first.");
        } else {
          showError(err || "Failed to delete stage");
        }
        break;
      }

      case "leadPipelines/searchLeads/rejected":
        showError("Lead search failed. Please try again.");
        break;

      // ===== LEADS META (TAGS) SLICE REJECTED =====

      case "leadsMeta/fetchTags/rejected":
        showError("Failed to load tags. Please refresh and try again.");
        break;

      case "leadsMeta/createTag/rejected": {
        const err = getErrorMessage(action);
        if (
          safeIncludes(err, "duplicate") ||
          safeIncludes(err, "already exists")
          ...
          showError(err || "Failed to create tag");
        }
        break;
      }

      case "leadsMeta/updateTag/rejected": {
        const err = getErrorMessage(action);
        showError(err || "Failed to update tag");
        break;
      }

      case "leadsMeta/deleteTag/rejected": {
        const err = getErrorMessage(action);
        showError(err || "Failed to delete tag");
        break;
      }

      // ===== LEADS (KANBAN) SLICE REJECTED =====

      case "leads/fetchPipelineLeads/rejected":
        showError("Failed to load leads. Please refresh and try again.");
        break;

      case "leads/createLead/rejected": {
        const err = getErrorMessage(action);
        if (safeIncludes(err, "validation")) {
          showError("Invalid lead data. Please check your input.");
        } else {
      ...
      case "leads/updateLeadStage/rejected": {
        const err = getErrorMessage(action);
        showError(err || "Failed to move lead. Please try again.");
        break;
      }

      // ===== LEAD TIMELINE SLICE REJECTED =====

      case "leadTimeline/fetchTimeline/rejected":
        showError("Failed to load timeline. Please try again.");
        break;

      case "leadTimeline/loadMoreTimeline/rejected":
        showError("Failed to load more items. Please try again.");
        break;

      case "leadTimeline/createComment/rejected": {
        const err = getErrorMessage(action);
        if (safeIncludes(err, "validation")) {
          showError("Invalid comment. Please check your input.");
        } else {
          showError(err || "Failed to add comment");
        }
        break;
      }

      case "leadTimeline/updateComment/rejected": {
        const err = getErrorMessage(action);
        showError(err || "Failed to update comment");
        break;
      }

      case "leadTimeline/deleteComment/rejected": {
        const err = getErrorMessage(action);
        showError(err || "Failed to delete comment");
        break;
      }

      case "leadTimeline/fetchStageHistory/rejected":
        showError("Failed to load stage history. Please try again.");
        break;

      // ===== LEADS INFLUENCER SLICE (leads-manager) REJECTED =====

      case "influencers/fetchList/rejected":
        showError("Failed to load influencers. Please refresh and try again.");
        break;

      case "influencers/search/rejected":
        showError("Influencer search failed. Please try again.");
        break;

      case "influencers/fetchById/rejected": {
        const err = getErrorMessage(action);
        if (safeIncludes(err, "not found")) {
          showError("Influencer not found. Please refresh the page.");
        } else {
          showError("Failed to load influencer details.");
        }
        break;
      }

      case "influencers/create/rejected": {
        const err = getErrorMessage(action);
        if (
          safeIncludes(err, "duplicate") ||
          safeIncludes(err, "already exists")
        ) {
          showError("This influencer already exists.");
        } else if (safeIncludes(err, "validation")) {
          showError("Invalid data. Please check your input.");
        } else {
          showError(err || "Failed to add influencer");
        }
        break;
      }

      case "influencers/update/rejected": {
        const err = getErrorMessage(action);
        if (safeIncludes(err, "not found")) {
          showError("Influencer not found. Please refresh the page.");
        } else {
          showError(err || "Failed to update influencer");
        }
        break;
      }

      case "influencers/delete/rejected": {
        const err = getErrorMessage(action);
        if (safeIncludes(err, "associated")) {
          showError("Cannot delete influencer: has associated data.");
        } else {
          showError(err || "Failed to delete influencer");
        }
        break;
      }
      ...
      case "taskCategory/toggleCategoryPin/rejected": {
        const err = getErrorMessage(action);
        if (safeIncludes(err, "Maximum 10")) {
          showError("Maximum 10 boards can be pinned");
        } else {
          showError(err || "Failed to toggle pin");
        }
        break;
      }

      case "taskCategory/fetchCategoryBoards/rejected":
        showError("Failed to load boards. Please refresh and try again.");
        break;
      ...
      case "users/sendPasswordResetLink/rejected":
        ...
        break;
      case "reconcile/exportTasks/rejected": {
        showError(action?.payload || "Failed to export tasks");
  ...
  if (action.type.endsWith("/pending")) {
    switch (action.type) {
      ...
      case "activities/updateActivityLifecycle/pending":
        showInfo("Updating activity status...");
        break;

      case "activities/updateActivityEmail/pending":
        showInfo("Saving email changes...");
        break;

      // ===== LEAD DETAILS SLICE PENDING =====

      case "leadDetails/updateLead/pending":
        showInfo("Saving lead changes...");
        break;

      case "leadDetails/updateLeadStage/pending":
        showInfo("Moving to new stage...");
        break;

      case "leadDetails/updateLeadAssignments/pending":
        showInfo("Updating assignments...");
        break;

      case "leadDetails/createLeadActivity/pending":
        showInfo("Creating activity...");
        break;

      case "leadDetails/updateLeadActivity/pending":
        showInfo("Updating activity...");
        break;

      case "leadDetails/updateActivityLifecycle/pending":
        showInfo("Updating activity status...");
        break;

      case "leadDetails/deleteActivity/pending":
        showInfo("Deleting activity...");
        break;

      case "leadDetails/updateActivityEmail/pending":
        showInfo("Saving email changes...");
        break;

      // ===== LEAD CONTACTS SLICE PENDING =====

      case "leadContact/create/pending":
        showInfo("Adding contact...");
        break;

      case "leadContact/update/pending":
        showInfo("Updating contact...");
        break;

      case "leadContact/delete/pending":
        showInfo("Deleting contact...");
        break;

      case "taskCategory/toggleCategoryPin/pending":
        showInfo("Updating board pin...");
        break;

      case "taskCategory/fetchCategoryBoards/pending":
        // Silent
        break;

      // ===== LEAD PIPELINES SLICE PENDING =====

      case "leadPipelines/create/pending":
        showInfo("Creating pipeline...");
        break;

      case "leadPipelines/update/pending":
        showInfo("Updating pipeline...");
        break;

      case "leadPipelines/delete/pending":
        showInfo("Deleting pipeline...");
        break;

      case "leadPipelines/deleteStage/pending":
        showInfo("Deleting stage...");
        break;

      // ===== LEADS META (TAGS) SLICE PENDING =====

      case "leadsMeta/createTag/pending":
        showInfo("Creating tag...");
        break;

      case "leadsMeta/updateTag/pending":
        showInfo("Updating tag...");
        break;

      case "leadsMeta/deleteTag/pending":
        showInfo("Deleting tag...");
        break;

      // ===== LEADS (KANBAN) SLICE PENDING =====

      case "leads/createLead/pending":
        showInfo("Creating lead...");
        break;

      // ===== LEAD TIMELINE SLICE PENDING =====

      case "leadTimeline/createComment/pending":
        showInfo("Adding comment...");
        break;

      // ===== LEADS INFLUENCER SLICE (leads-manager) PENDING =====

      case "influencers/create/pending":
        showInfo("Adding influencer...");
        break;

      case "influencers/update/pending":
        showInfo("Updating influencer...");
        break;

      case "influencers/delete/pending":
