...
export const exportReconcileTasks = createAsyncThunk(
  "reconcile/exportTasks",
  async ({ filters }, { rejectWithValue }) => {
    try {
      ...
      const response = await fetch(
        `/api/admin_ops/tasks/reconcile/export?${params.toString()}`,
        {
          method: "POST",
        },
      );
      ...
      const fileName =
        disposition?.match(/filename="?([^"]+)"?/)?.[1] || "tasks-charges.xlsx";
      ...
      return rejectWithValue(error?.message || "Failed to export tasks");
    }
  },
);
...
const reconcileSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    // exporing reducers
    builder
      .addCase(exportReconcileTasks.pending, (state) => {
        state.loading.export = true;
      })
      .addCase(exportReconcileTasks.fulfilled, (state) => {
        state.loading.export = false;
      })
      .addCase(exportReconcileTasks.rejected, (state) => {
        state.loading.export = false;
      });
