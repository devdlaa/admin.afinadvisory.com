...
function formFromInitialData(data) {
...
export default function ReminderDialog({
  ...
  // ── fetch detail on open (update mode) ──
  useEffect(() => {
    if (isUpdate && activeReminderId) {
      dispatch(fetchReminderDetail(activeReminderId));
    }
  }, [isUpdate, activeReminderId]);

  // ── seed transformed data ──
  const initialData = isUpdate ? transformReminderDetail(detailData) : null;

  // ── form state ──
  const [form, setForm] = useState(() =>
    formFromInitialData(isUpdate ? initialData : null),
  );
  const savedRef = useRef(formFromInitialData(isUpdate ? initialData : null));

  const seededRef = useRef(false);
  if (isUpdate && initialData && !seededRef.current) {
    const seeded = formFromInitialData(initialData);
    setForm(seeded);
  ...
  const locked = loading || (isUpdate && detailLoading);
  ...
  return (
    <>
      ...
      >
        {/* ═══ LEFT CARD ═══ */}
        <div className={`${styles.card} ${styles.card_height_fill}`}>
          {snoozeOpen ? (
            <>
