...
export function buildUpdatePayload(current, saved) {
  const patch = {};
  ...
  if (current.description !== saved.description)
    patch.description = current.description?.trim() || null;

  if (current.bucket?.id !== saved.bucket?.id)
    patch.bucket_id = current.bucket?.id || null;

  if (current.assignee?.id !== saved.assignee?.id)
    patch.assigned_to = current.assignee?.id || null;

  if (JSON.stringify(current.tags) !== JSON.stringify(saved.tags))
    patch.tag_ids = current.tags.map((t) => t.id);

  const rcChanged =
    JSON.stringify(current.reminderConfig) !== JSON.stringify(saved.reminderConfig);

  if (rcChanged) {
    const rc = current.reminderConfig;
    patch.due_at                 = rc?.date                  ?? null;
    patch.is_recurring           = rc?.isRecurring           ?? false;
    patch.recurrence_type        = rc?.recurrence_type       ?? null;
    patch.recurrence_every       = rc?.recurrence_every      ?? null;
    patch.recurrence_end         = rc?.recurrence_end        ?? null;
    patch.recurrence_ends_after  = rc?.recurrence_ends_after ?? null;
    patch.week_days              = rc?.week_days             ?? null;
    patch.repeat_by              = rc?.repeat_by             ?? null;
  }
...
export const SNOOZE_OPTS = [
  { label: "15 min", payload: { action: "SNOOZE", duration_minutes: 15 } },
  { label: "30 min", payload: { action: "SNOOZE", duration_minutes: 30 } },
  { label: "45 min", payload: { action: "SNOOZE", duration_minutes: 45 } },
  { label: "1 hour", payload: { action: "SNOOZE", duration_minutes: 60 } },
  {
    label: "Tomorrow",
    payload: () => {
      const d = new Date();
      ...
      d.setHours(9, 0, 0, 0);
      return { action: "SNOOZE", snoozed_until: d.toISOString() };
    },
  },
  {
    label: "Next week",
