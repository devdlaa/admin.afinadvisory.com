...
const TaskStatusBoard = ({ statusCounts, loading = false }) => {
  ...
  const safeGlobal = global && typeof global === "object" ? global : {};
  const safeFiltered = filtered && typeof filtered === "object" ? filtered : {};

  const globalTotal = Object.values(safeGlobal).reduce(
    (sum, count) => sum + (count || 0),
    0
  );

  const filteredTotal = Object.values(safeFiltered).reduce(
    (sum, count) => sum + (count || 0),
    0
  );
      ...
      <div className="task-status-board">
        <div className="status-columns">
          ...
          <div className="status-column total-column">
            ...
            <div
              className={`column-count ${
                hasActiveFilters ? "has-filtered" : ""
              }`}
            >
              {globalTotal}
            </div>
            {hasActiveFilters && (
              <div className="filtered-badge">{filteredTotal} </div>
            )}
          </div>
          ...
          <div className="status-column pending-column">
            ...
            {hasActiveFilters && (
              <div className="filtered-badge">{filtered.PENDING || 0}</div>
            )}
          </div>
          ...
          <div className="status-column progress-column">
            ...
            {hasActiveFilters && (
              <div className="filtered-badge">{filtered.IN_PROGRESS || 0}</div>
            )}
          </div>
          ...
          <div className="status-column completed-column">
            ...
            {hasActiveFilters && (
              <div className="filtered-badge">{filtered.COMPLETED || 0}</div>
            )}
          </div>
          ...
          <div className="status-column hold-column">
            ...
            {hasActiveFilters && (
              <div className="filtered-badge">{filtered.ON_HOLD || 0}</div>
            )}
          </div>
          ...
          <div className="status-column client-column">
            ...
            {hasActiveFilters && (
              <div className="filtered-badge">
                {filtered.PENDING_CLIENT_INPUT || 0}
              </div>
            )}
          </div>
          ...
          <div className="status-column cancelled-column">
            ...
            {hasActiveFilters && (
              <div className="filtered-badge">{filtered.CANCELLED || 0}</div>
            )}
          </div>
        </div>
      </div>
