...
import LinkSelectionDialog from "../leads-manager/components/LinkSelectionDialog/LinkSelectionDialog.jsx";
...
import {
  quickSearchEntities,
  selectQuickSearchResults as selectRefEntityResults,
  selectEntityLoadingStates as selectRefEntityLoadingStates,
...
import { getFinancialYearOptions } from "@/utils/client/cutils.js";
...
const STANDARD_FILTER_KEYS = [
  ...
  "financial_year",
  "referred_by_type",
  "referred_by_search_field",
  "referred_by_display_name",
  "referred_by_display_meta",
...
function TasksPageContent() {
  ...
  const refEntityResults = useSelector(selectRefEntityResults);
  const refEntityLoadingStates = useSelector(selectRefEntityLoadingStates);
  const refEntitySearching = refEntityLoadingStates?.quickSearch ?? false;
  const refInfluencerResults = useSelector(selectInfluencerSearchList);
  const refInfluencerSearching = useSelector(selectInfluencerSearchLoading);
  ...
  const [showReferenceFilter, setShowReferenceFilter] = useState(false);
  ...
  const selectedReferenceFilterData = useMemo(() => {
    const type = currentFilters.referred_by_type;
    const field = currentFilters.referred_by_search_field;
    const displayName = currentFilters.referred_by_display_name || field;
    const displayMeta = currentFilters.referred_by_display_meta || "";
    if (!type || !field) return null;

    if (type === "entity" || type === "influencer") {
      return { __type: type, id: field, displayName, displayMeta };
    }
    if (type === "external") {
      // re-hydrate the correct field so ExternalPanel shows it in the right input
      const isEmail = displayMeta === "Email";
      const isPhone = displayMeta === "Phone";
      return {
        __type: "external",
        name: !isEmail && !isPhone ? field : "",
        email: isEmail ? field : "",
        phone: isPhone ? field : "",
        displayName,
        displayMeta,
    ...
    currentFilters.referred_by_type,
    currentFilters.referred_by_search_field,
    currentFilters.referred_by_display_name,
    currentFilters.referred_by_display_meta,
  ]);
  ...
  const filterDropdowns = useMemo(
    () => [
      ...
      {
        filterKey: "financial_year",
        label: "Financial Year",
        placeholder: "Select FY",
        icon: CalendarHeart,
        options: getFinancialYearOptions ? getFinancialYearOptions() : [],
        enableLocalSearch: false,
      },
    ],
    [
  ...
  const handleClearAllFilters = useCallback(() => {
    dispatch(
      setFilters({
        ...
        referred_by_search_field: null,
        referred_by_display_name: null,
        referred_by_display_meta: null,
        financial_year: null,
      }),
    );
    dispatch(fetchTasks(true));
  ...
  const handleShowReferenceFilter = useCallback(() => {
    setShowReferenceFilter(true);
  }, []);

  const handleReferenceFilterSearch = useCallback(
    (query, tabId) => {
      if (!query.trim()) return;
      if (tabId === "entity") {
        dispatch(quickSearchEntitiesRef({ search: query, limit: 20 }));
      }
      if (tabId === "influencer") {
  ...
  const handleReferenceFilterTabChange = useCallback(() => {
    // nothing needed — redux results are keyed by slice, not tab
  }, []);

  const handleReferenceFilterConfirm = useCallback(
    (payload) => {
      if (payload.type === "__cleared") {
      ...
      } else if (payload.type === "entity" || payload.type === "influencer") {
        dispatch(
          setFilters({
            referred_by_type: payload.type,
            referred_by_search_field: payload.id,
            referred_by_display_name: payload.displayName ?? "",
            referred_by_display_meta: payload.displayMeta ?? "",
          }),
        );
        ...
        dispatch(
          setFilters({
            referred_by_type: "external",
            referred_by_search_field: searchField,
            referred_by_display_name: searchField,
            referred_by_display_meta: payload.email?.trim()
              ? "Email"
              : payload.phone?.trim()
                ? "Phone"
                : "Name",
      ...
      <div className={style.tasksPage}>
        <TaskActionBar
          ...
          activeFilters={{
            ...
            financial_year: currentFilters.financial_year,
          }}
          ...
          onShowReferenceFilter={handleShowReferenceFilter}
          isReferenceFilterOpen={showReferenceFilter}
          isReferenceFilterActive={
            !!currentFilters.referred_by_type &&
            !!currentFilters.referred_by_search_field
          }
        />
        ...
        <LinkSelectionDialog
          ...
          tabs={[
            {
              ...
              results: refEntityResults.map((e) => ({
                ...e,
                displayName: e.name,
                displayMeta: [
              ...
              isSearching: refEntitySearching,
            },
            {
              id: "influencer",
              label: "Influencer",
              ...
              results: refInfluencerResults.map((e) => ({
                ...e,
                displayName: e.name,
                displayMeta: [e.email, e.phone].filter(Boolean).join(" • "),
              })),
              isSearching: refInfluencerSearching,
            },
            {
              id: "external",
              label: "External Ref",
          ...
          selectedData={selectedReferenceFilterData}
          onClose={() => setShowReferenceFilter(false)}
          onSearchChange={handleReferenceFilterSearch}
          onTabChange={handleReferenceFilterTabChange}
          onConfirm={handleReferenceFilterConfirm}
          isUpdating={false}
        />
      </div>
    </ForceNotification>
