...
const ClientAddUpdateDialog = forwardRef(({ mode, client }, ref) => {
  ...
  const handleCustomFieldChange = (index, field, value) => {
    ...
    // Clear validation error for custom fields
    if (validationErrors.custom_fields) {
      setValidationErrors((prev) => {
        const newErrors = { ...prev };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const cleanedData = Object.entries(formData).reduce(
        (acc, [key, value]) => {
          if (key === "custom_fields") {
            // Filter out empty custom fields and remove isCustomName flag
            ...
            acc[key] = validFields.length > 0 ? validFields : undefined;
          } else {
      ...
      if (err.errors) {
        ...
        //  errors will be handled by toast middleware
                              ...
                              <select
                                ...
                                onChange={(e) => {
                                  ...
                                  if (value === "Custom") {
                                    ...
                                    handleCustomFieldChange(index, "name", "");
                                  } else {
                                    handleCustomFieldChange(
                                      index,
                                      "name",
