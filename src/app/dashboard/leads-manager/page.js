...
import LeadTagsDialog from "../reminders/components/LeadTagsDialog/LeadTagsDialog";
...
export default function LeadsManagerPage() {
  ...
  // Create lead — alert for now, will open drawer later
  const handleCreateLead = useCallback((stage) => {
    alert(`Create New Lead in: ${stage.name}`);
  }, []);
  ...
  const handleDeleteStage = useCallback(
    (stageId) => {
      ...
      if (!stage || stage.is_closed) return; // system stages cannot be deleted
  ...
  const stageOptions = activePipelineStages.map((s) => s.name);
