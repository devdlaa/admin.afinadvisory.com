...
const FilterSchema = z.object({
  ...
  startDate: z
    .string()
    .refine((s) => !isNaN(new Date(s).getTime()), { message: "Invalid start date" })
    .optional(),
  endDate: z
    .string()
    .refine((s) => !isNaN(new Date(s).getTime()), { message: "Invalid end date" })
    .optional(),
  extraFilter: z
    .object({
      field: z.enum(["couponCode", "customerId", "influencerId", "service_booking_id"]),
      value: z.string(),
    })
    .optional(),
});
...
export async function POST(req) {
  ...
  try {
    ...
    console.log("Zod validation result:", { success: parsed.success, error: parsed.error });
    ...
    if (quickRange && (startDate || endDate)) {
      return NextResponse.json(
        { success: false, error: "Use either quickRange OR custom dates, not both." },
        { status: 400 }
      );
    }
    ...
    console.error("commissions filter API error:", err);
    return NextResponse.json(
